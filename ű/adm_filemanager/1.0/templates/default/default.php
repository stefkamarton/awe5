...
function uploadForm($array) {
    ...
    echo "<form id='fileupload' method='post' enctype='multipart/form-data' data-progressbar='#main-bar' data-comid='" . $array['config']['url_id'] . "' data-waiting='0' data-method='fileupload' data-url='" . ADM_FILEMANAGER_AJAX_VIEW["url"] . "'>
...
function createWindow($array) {
    if (!empty($array)) {
        ...
        echo "<div class='file-list-in' id='" . $array['config']['url_id'] . "_" . substr(ADM_FILEMANAGER_AJAX_VIEW['result'], 1) . "'>";
...
function recursiveDirectoryTreeWriter($array, $path = "") {
    ...
    foreach ($array['directory_tree'] as $key => $value) {
        if (!empty($value)) {
            if ($key == "/") {
                $str .= "<li class='expanded-directory'><div class='expanded-btn'><i class='fas fa-angle-right'></i></div><div class='folder-icon'><i class='fas fa-folder'></i></div><form data-comid='" . $array['config']['url_id'] . "' data-waiting='" . $array['config']['waiting'] . "' data-method='" . ADM_FILEMANAGER_AJAX_VIEW["method2"] . "' data-url='" . ADM_FILEMANAGER_AJAX_VIEW["url"] . "' class='folder-name'>" . $key . "<input style='display:none;' name='path' type='text' value='' readOnly /></form></li>";
                $str .= "<ul class='tree-view expanded'>" . recursiveDirectoryTreeWriter(array_merge($array, array("directory_tree" => $value))) . "</ul>";
            } else {
            ...
            $str .= "<li class='alone'><div class='folder-icon'><i class='fas fa-folder'></i></div><form class='folder-name' data-comid='" . $array['config']['url_id'] . "' data-waiting='" . ADM_FILEMANAGER_AJAX_VIEW['waiting'] . "' data-method='" . ADM_FILEMANAGER_AJAX_VIEW["method2"] . "' data-url='" . ADM_FILEMANAGER_AJAX_VIEW["url"] . "'>" . $key . "<input style='display:none;' name='path' type='text' value='" . $path . "/" . $key . "' readOnly /></form></li>";
        }
    }
    return $str;
}
